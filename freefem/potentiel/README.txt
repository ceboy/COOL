potential
  ...
  linear water wave model "d_t eta = DNO(phi)" at the surface of a domain where "-Delta phi = 0", "d_t phi = -gravity*eta"

  is complemented by various BC (mixed Dirichlet-Neumann-Periodic BVP for the scalar poisson equation) 

  in various polygonal geometries (also various meshes) with
...
=>primal-mixed (P1-P0^2: undefinite saddle-point equivalent to primal after static condensation can be solved by UMFPACK or GMRES without penalization)
 ...
 RUN:
  * potential*.edp: according to the variational formulation of the problem retained

 POST:
  * potential*.gp: gnuplot script to show *.dat
