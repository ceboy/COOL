Poisson problem
  ...
  BVP for the scalar Laplace equation "-Delta u = f" with mixed Dirichlet-Neumann-Periodic BC

  solved in various polygonal geometries (also various meshes for each geomtry) with

  data from various smooth analytical solutions (allow for error estimation: no smoothing property in general) and
  ...
  and (primal) non-conforming ones with Crouzeix-Raviart P1nc FE space.
  ...
  * poisson*.edp: according to the variational formulation of the problem retained
  > trace: evaluated "strongly" for potential in H^1 (primal), "weakly" for dual -- nc ?
  > normal trace: evaluated "weakly" for primal, "strongly" for flux in H(div) (dual) -- nc ?
  > periodic BC: weak error = "macthing" for glued trial / "residual" for glued test
    (nothing to say for strong error: as above)

 POST:
  * poisson*.gp: gnuplot script showing *.dat (to be run in Result/... where corresponding file *.dat lies)
  * hcv*.edp to read output files and recompute postprocessed values

 TODO:
  * finish validations: in particular, non-conforming approximation (Crouzeix-Raviart P1nc equivalent with RT0), and problem for normal trace convergence with mixed-dual on geo3 !!
